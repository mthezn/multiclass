...
mask_dirs_val = ["MICCAI/instrument_2017_test/instrument_2017_test/instrument_dataset_4/TypeSegmentationRescaled/"]
...
autosam_checkpoint = "checkpoints/01_09/autoSamMultiUnet4ZCV7.pth"
...
for i in range(len(datasetTest)):
    _, mask = datasetTest[i]   # mask è un torch.Tensor (H,W)
...
for i,(images, labels) in enumerate(dataloaderTest):  # i->batch index, images->batch of images, labels->batch of labels
    ...
    for j,( image, label) in enumerate(zip(images, labels)):
     ...
     # manca batch dimensione
        ...
        mask = torch.argmax(low_res.detach().cpu(), axis=1)
       ...
       # mask = mask.to(torch.uint8)
        ...
        # immagine originale (da tensor a numpy per plt)
        img_vis = image.squeeze(0).cpu().permute(1, 2, 0).numpy()
        img_vis = (img_vis - img_vis.min()) / (img_vis.max() - img_vis.min())  # normalizzazione [0,1]
        ...
        # salva
        save_path = os.path.join(save_dir, f"result_{i}_{j}.png")
...
print(datasetTest.getInstruments())
